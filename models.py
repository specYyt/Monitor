...
class Service(models.Model):
    ...
    has_sub_service = models.BooleanField('ServiceIndex',help_text=u"如果一个服务还有独立的子服务，比如，网卡服务有多个独立的子网卡")
...
class ServiceIndex(models.Model):
    name = models.CharField(max_length=64) #指标名称，可以随便取，不在程序中用于判断
    key =models.CharField(max_length=64) #指标名，这个必须为固定的指标值，此值必须与插件值一致，否则无法匹配
    data_type_choices = (
        ('int',"int"),
        ('float',"float"),
    )
...
class TriggerExpression(models.Model):
    ...
    '''
    此处实现两条记录关联的方法：
        每一个触发器表达式在创建的时候都会关联触发器、服务、服务指标等这些项目，据此可将触发器记录关联起来，进行and 或 or 的判断
    '''
...
class Action(models.Model):
    ...
    triggers = models.ManyToManyField('Trigger',blank=True,help_text=u"想让哪些trigger触发")
    interval = models.IntegerField(u'告警间隔(s)',default=300)
    operations = models.ManyToManyField('ActionOperation') #告警的动作，如展示、发邮件、发短信等
...
#定义告警的方式，实现告警升级 ===》通过 step(告警次数) 来实现告警升级
class ActionOperation(models.Model):
    ...
    msg_format = models.TextField(u"消息格式",default=_msg_format) #实现字符串格式化
